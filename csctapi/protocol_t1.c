...
static void Protocol_T1_Clear(Protocol_T1 * t1);

static int Protocol_T1_SendBlock(Protocol_T1 * t1, T1_Block * block);

static int Protocol_T1_ReceiveBlock(Protocol_T1 * t1, T1_Block ** block);

static int Protocol_T1_UpdateBWT(Protocol_T1 * t1, unsigned short bwt);

/*
 * Exproted funtions definition
 */

Protocol_T1 *Protocol_T1_New(void)
{
	Protocol_T1 *t1;
...
int Protocol_T1_Init(Protocol_T1 * t1, ICC_Async * icc, PPS_ProtocolParameters * params)
{
	...
	printf("Protocol: T=1: IFSC=%d, IFSD=%d, CWT=%d, BWT=%d, BGT=%d, EDC=%s\n", t1->ifsc, t1->ifsd, t1->cwt, t1->bwt, t1->bgt, (t1->edc == PROTOCOL_T1_EDC_LRC) ? "LRC" : "CRC");
#endif

	return PROTOCOL_T1_OK;
}

int Protocol_T1_Command(Protocol_T1 * t1, APDU_Cmd * cmd, APDU_Rsp ** rsp)
{
	T1_Block *block;
	...
	while ((ret == PROTOCOL_T1_OK) && more) {
		...
		if (ret == PROTOCOL_T1_OK) {
			...
			if (rsp_type == T1_BLOCK_R_OK) {
				...
				/* Create an I-Block */
				block = T1_Block_NewIBlock(bytes, APDU_Cmd_Raw(cmd) + counter, t1->ns, more);
#ifdef DEBUG_PROTOCOL
...
int Protocol_T1_Close(Protocol_T1 * t1)
{
	Protocol_T1_Clear(t1);
...
void Protocol_T1_Delete(Protocol_T1 * t1)
{
	free(t1);
}
...
static int Protocol_T1_ReceiveBlock(Protocol_T1 * t1, T1_Block ** block)
{
	...
	if (ICC_Async_Receive(t1->icc, 4, buffer) != ICC_ASYNC_OK) {
		...
		if (buffer[2] != 0x00) {
			...
			if (ICC_Async_Receive(t1->icc, buffer[2], buffer + 4) != ICC_ASYNC_OK) {
				(*block) = NULL;
				ret = PROTOCOL_T1_ICC_ERROR;
			}
