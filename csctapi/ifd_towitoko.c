...
#ifdef DEBUG_IFD
	printf("IFD: Parity = %s\n", parity == IFD_TOWITOKO_PARITY_ODD ? "Odd" : parity == IFD_TOWITOKO_PARITY_EVEN ? "Even" : "Invalid");
#endif
...
#endif
	{
		...
		switch (reader_serial_card_detect & 0x7f) {
			case 0:
				...
				break;
			case 1:
				in = (modembits & TIOCM_DSR);
				break;
			case 2:
				in = (modembits & TIOCM_CTS);
				break;
			case 3:
				in = (modembits & TIOCM_RNG);
				break;
			default:
				in = 0;	// dummy
		}
...
#endif
		// if we get here we have a valid ATR and can init the IO->SmartReaderConf strcture.
...
BYTE IFD_Towitoko_GetFirmware(IFD * ifd)
{
	return ifd->firmware;
}

BYTE IFD_Towitoko_GetSlot(IFD * ifd)
{
	return ifd->slot;
}

unsigned IFD_Towitoko_GetNumSlots(IFD * ifd)
{
	return 1;
}
...
static int IFD_Towitoko_GetReaderInfo(IFD * ifd)
{
	...
	printf("IFD: Reader type = %s\n",
	       ...
	       status[0] == IFD_TOWITOKO_CHIPDRIVE_MICRO ? "Chipdrive Micro" : status[0] == IFD_TOWITOKO_KARTENZWERG_II ? "Kartenzwerg II" : status[0] == IFD_TOWITOKO_MULTICAM ? "Multicam" : status[0] == IFD_TOWITOKO_KARTENZWERG ? "Kartenzwerg" : "Unknown");
#endif
