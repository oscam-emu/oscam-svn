...
static int Protocol_T0_ExchangeTPDU(Protocol_T0 * t0, APDU_Cmd * cmd, APDU_Rsp ** rsp)
{
	...
	while (recv < PROTOCOL_T0_MAX_SHORT_RESPONSE) {
		...
		if (buffer[recv] == 0x60) {
			...
			continue;
		} else if ((buffer[recv] & 0xF0) == 0x60 || (buffer[recv] & 0xF0) == 0x90) {	/* SW1 byte received *///printf("sw1\n");
			recv++;
			...
			ret = PROTOCOL_T0_OK;
			break;
		} else if ((buffer[recv] & 0x0E) == (APDU_Cmd_Ins(cmd) & 0x0E)) {	/* ACK byte received *///printf("ack\n");
			...
			if (cmd_case == APDU_CASE_2S) {
				...
				if (ICC_Async_Receive(t0->icc, MAX(Le - recv, 0), buffer + recv) != ICC_ASYNC_OK) {	//printf("error %d\n", (int)Le);
					ret = PROTOCOL_T0_ICC_ERROR;
					break;
				}
