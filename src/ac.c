...
static int ac_dw_weight(ECM_REQUEST * er)
{
	...
	for (cpmap = cfg->cpmap; (cpmap); cpmap = cpmap->next)
		if ((cpmap->caid == 0 || cpmap->caid == er->caid) &&
		    ...
		    (cpmap->sid == 0 || cpmap->sid == er->srvid) && (cpmap->chid == 0 || cpmap->chid == er->chid))
...
{
	if (!client[cs_idx].ac_limit || !cfg->ac_enabled)
		return;

	if (level == 1) {
		if (er->rc == 7)
			acasc[client[cs_idx].ac_idx].count++;
		if (er->rc > 3)
			return;	// not found
