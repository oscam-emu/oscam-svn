	...
	case 0xB6:
		{
			...
			cs_log("nano B6: modexp %d%d%d%d%d%d: %02d/%02d/%04d", (m & 0x20) ? 1 : 0,
			       (m & 0x10) ? 1 : 0, (m & 0x08) ? 1 : 0, (m & 0x04) ? 1 : 0, (m & 0x02) ? 1 : 0,
			       (m & 0x01) ? 1 : 0, vd.day_s, vd.month_s, vd.year_s + 1980);
			break;
		}
	}
}
...
//  }
	...
	insb8[4] = 0x07;
	read_cmd(insb8, NULL);	// read unique id
...
//  cs_log("type: viaccess, ver: %s serial: %llu", ver, b2ll(5, cta_res+2));
	...
	while ((cta_res[cta_lr - 2] == 0x90) && (cta_res[cta_lr - 1] == 0)) {
		insc0[4] = 0x1a;
		read_cmd(insc0, NULL);	// show provider properties
		cta_res[2] &= 0xF0;
...
int viaccess_do_emm(EMM_PACKET * ep)
{
	...
	for (emmUpToEnd = emmLen; (emmParsed[1] != 0) && (emmUpToEnd > 0);
	     emmUpToEnd -= (2 + emmParsed[1]), emmParsed += (2 + emmParsed[1])) {
		...
		if (emmParsed[0] == 0x90 && emmParsed[1] == 0x03) {
			...
			if (chk_prov(ident, keynr)) {
				...
				cs_debug("smartcardviaccess emm: provider or key not found on card (%x, %x)", ident,
					 keynr);
				return 0;
			}
	...
	if (!nano92Data) {
		...
		if (cta_res[cta_lr - 2] != 0x90 || cta_res[cta_lr - 1] != 0x00) {
			...
			if (cta_res[0] != 0x00 || cta_res[1] != 00 || cta_res[cta_lr - 2] != 0x90
			    || cta_res[cta_lr - 1] != 0x00) {
...
int viaccess_card_info(void)
{
	...
	for (i = 1; (cta_res[cta_lr - 2] == 0x90) && (cta_res[cta_lr - 1] == 0); i++) {
		...
		insb8[4] = 0x02;
		read_cmd(insb8, NULL);	// read name nano + len
		l = cta_res[1];
		insb8[4] = l;
		read_cmd(insb8, NULL);	// read name
		cta_res[l] = 0;
		trim((char *) cta_res);
		if (cta_res[0])
		...
		insb8[4] = 0x02;
		read_cmd(insb8, NULL);	// read GEO nano + len
		l = cta_res[1];
		insb8[4] = l;
		read_cmd(insb8, NULL);	// read geo
		...
		while ((cta_res[cta_lr - 2] == 0x90) && (cta_res[cta_lr - 1] == 0)) {
			insb8[4] = 0x02;
			read_cmd(insb8, NULL);	// read class subs nano + len
			if ((cta_res[cta_lr - 2] == 0x90) && (cta_res[cta_lr - 1] == 0)) {
				int fshow;
				l = cta_res[1];
				...
				fshow = (scls < show_cls);
				insb8[4] = l;
				read_cmd(insb8, NULL);	// read class subs
