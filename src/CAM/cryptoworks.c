			...
			for (int i = 0; i < l && r < 2;) {
				...
				case 0xD4:
					de(printf("smartcardcryptoworks: nano D4 (rand)\n"))
						...
						di(printf
						   ("smartcardcryptoworks: random data check failed after decrypt\n"))
						    break;
				case 0xDB:	// CW
				...
				case 0xDF:	// signature
					de(printf("smartcardcryptoworks: nano DF %02x (sig)\n", n))
					    if (n == 0x08) {
						if ((buff[i + 2] & 0x50) == 0x50 && !(buff[i + 3] & 0x01)
						    && (buff[i + 5] & 0x80))
							r |= 2;
						...
						if (ucpkValid) {
							...
							di(printf
							   ("smartcardcryptoworks: valid UCPK needed for camcrypt!\n"))
							    return false;
						}
					}
					break;
		...
		if (cta_res[cta_lr - 2] == 0x9f) {
			...
			for (i = 0; i < secLen && r < 2;) {
				...
				case 0xDF:	// signature
					...
					if (n == 0x08) {
						if ((cta_res[i + 2] & 0x50) == 0x50 && !(cta_res[i + 3] & 0x01)
						    && (cta_res[i + 5] & 0x80))
							r |= 2;
					} else if (n == 0x40)	// camcrypt
					{
		...
		if ((cta_res[cta_lr - 2] == 0x9f) && (cta_res[cta_lr - 1] == 0x1c)) {
			...
			if ((cta_lr > 26) && (cta_res[cta_lr - 2] == 0x90) && (cta_res[cta_lr - 1] == 0)) {
				if (rc = (((cta_res[20] & 0x50) == 0x50) &&
					  (!(cta_res[21] & 0x01)) && (cta_res[23] & 0x80)))
