...
int conax_card_init(uchar * atr, int atrsize)
{
	...
	if ((n = read_record(ins26, ins26 + 5)) < 0)
	...
	for (i = 0; i < n; i += cta_res[i + 1] + 2)
		...
		case 0x20:
			...
			break;
		case 0x28:
			reader[ridx].caid[0] = (cta_res[i + 2] << 8) | cta_res[i + 3];
		}

	if ((n = read_record(ins82, ins82 + 5)) < 0)
			...
			for (i = 0; i < cta_lr - 2; i += cta_res[i + 1] + 2)
				...
				case 0x31:
					if ((cta_res[i + 1] == 0x02 && cta_res[i + 2] == 0x00 && cta_res[i + 3] == 0x00)
					    || (cta_res[i + 1] == 0x02 && cta_res[i + 2] == 0x40
						&& cta_res[i + 3] == 0x00))
						break;
		...
		while (cta_res[cta_lr - 2] == 0x98) {
			...
			if ((cta_res[cta_lr - 2] == 0x90) || (cta_res[cta_lr - 2] == 0x98)) {
				for (j = 0; j < cta_lr - 2; j += cta_res[j + 1] + 2) {
					...
					for (k = 0, i = j + 4 + type; (i < j + cta_res[j + 1]) && (k < 2);
					     i += cta_res[i + 1] + 2) {
						int l;
						switch (cta_res[i]) {
						case 0x01:
							l = (cta_res[i + 1] < (sizeof(provname) - 1)) ?
