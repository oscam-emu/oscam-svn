...
0x02, 0x09, 0x03, 0x00, 0x40,
	    ...
	    0xA4, 0x1F, 0x8E, 0x47, 0xD0, 0x24, 0x11, 0xD0}, sc_GetCamKey384DZ[] = {
0x02, 0x09, 0x03, 0x00, 0x40,
	    ...
	    0x8A, 0x08, 0x7F, 0x0B, 0xB8, 0x79, 0xC3, 0x5D}, sc_GetCamKey384FZ[] = {
0x02, 0x09, 0x03, 0x00, 0x40,
	    ...
	    0x95, 0xA9, 0x64, 0x22, 0x5E, 0xA4, 0xAF, 0xA9}, sc_GetCamKey383C[] = {
0x02, 0x09, 0x03, 0x00, 0x40,
...
int irdeto_card_init(uchar * atr, int atrlen)
{
	...
	cs_ri_log("type: %s, caid: %04X, acs: %x.%02x%s",
		  (nagra) ? "aladin" : "irdeto", reader[ridx].caid[0], cta_res[0], cta_res[1], buf);
...
//      if ((cta_lr==26) && (cta_res[0]!=0xf))
			if ((cta_lr == 26) && ((!(i & 1)) || (cta_res[0] != 0xf))) {
				...
				sprintf((char *) buf + strlen((char *) buf), ",%06lx",
					b2i(3, &reader[ridx].prid[i][1]));
			} else
