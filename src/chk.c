	...
	if (caid == er->caid) {
		for (i = 0; (!rc) && (i < reader[ridx].nprov); i++) {
			prid = (ulong) ((reader[ridx].prid[i][0] << 16) |
					(reader[ridx].prid[i][1] << 8) | (reader[ridx].prid[i][2]));
