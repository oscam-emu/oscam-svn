...
static int shmsize = CS_ECMCACHESIZE * (sizeof(struct s_ecm)) +
    CS_MAXPID * (sizeof(struct s_client)) + CS_MAXREADER * (sizeof(struct s_reader)) +
#ifdef CS_WITH_GBOX
    CS_MAXCARDS * (sizeof(struct card_struct)) +
    CS_MAXIGNORE * (sizeof(long)) + CS_MAXPID * (sizeof(struct idstore_struct)) +
#endif
#ifdef CS_ANTICASC
		...
		default:	// HERE is master
			...
			if (ip) {
				...
				case 99:
					...
					if (reader[ridx].r_port)
						...
						if (reader[ridx].typ == R_MOUSE || reader[ridx].typ == R_SMART)
							cs_log
							    ("reader started (pid=%d, device=%s, detect=%s%s, mhz=%d)",
							     pid, reader[ridx].device,
							     reader[ridx].detect & 0x80 ? "!" : "",
							     RDR_CD_TXT[reader[ridx].detect & 0x7f], reader[ridx].mhz);
						else
				...
				case 98:
					...
					client[i].ip = client[0].ip;
					strcpy(client[i].usr, client[0].usr);
					cs_log("resolver started (pid=%d, delay=%d sec)", pid, cfg->resolvedelay);
					cdiff = i;
					break;
				case 97:
				...
				case 96:
					...
					client[i].ip = client[0].ip;
					strcpy(client[i].usr, client[0].usr);
					cs_log("anticascader started (pid=%d, delay=%d min)", pid, cfg->ac_stime);
					cdiff = i;
					break;
#endif
				default:
...
static void init_shm()
{
	...
	printf("SHM ALLOC: %x\n", shmsize);
	printf("SHM START: %p\n", (void *) ecmcache);
	printf("SHM ST1: %p %x (%x)\n", (void *) ecmidx, ((void *) ecmidx) - ((void *) ecmcache),
	       CS_ECMCACHESIZE * (sizeof(struct s_ecm)));
...
static int start_listener(struct s_module *ph, int port_idx)
{
	...
	if (cfg->netprio)
		if (!setsockopt
		    (ph->ptab->ports[port_idx].fd, SOL_SOCKET, SO_PRIORITY, (void *) &cfg->netprio, sizeof(ulong)))
	...
	while (timeout--) {
		if (bind(ph->ptab->ports[port_idx].fd, (struct sockaddr *) &sad, sizeof(sad)) < 0) {
			if (timeout) {
				cs_log("%s: Bind request failed, waiting another %d seconds", ph->desc, timeout);
				sleep(1);
			} else {
	...
	cs_log("%s: initialized (fd=%d, port=%d%s%s%s)",
	       ...
	       ph->ptab->ports[port_idx].s_port, ptxt[0], ptxt[1], ph->logtxt ? ph->logtxt : "");
...
static void cs_fake_client(char *usr, int uniq, in_addr_t ip)
{
	...
	for (i = cdiff + 1; i < CS_MAXPID; i++) {
		if (client[i].pid && (client[i].typ == 'c')
			...
			client[cs_idx].dup = 1;
			client[cs_idx].au = -1;
			cs_log("client(%d) duplicate user '%s' from %s set to fake (uniq=%d)", cs_idx - cdiff, usr,
			       cs_inet_ntoa(ip), uniq);
			break;
		}
	}
}
...
int cs_auth_client(struct s_auth *account, char *e_txt)
{
	...
	case 0:		// reject access
		...
		cs_log("%s %s-client %s%s (%s)",
		       ...
		       client[cs_idx].ip ? t_reject : t_reject + 1, e_txt ? e_txt : t_msg[rc]);
		break;
	default:		// grant/check access
		...
		if (!rc) {
			...
			if (client[cs_idx].typ == 'c') {
				...
				client[cs_idx].pcrc =
				    crc32(0L, MD5((uchar *) account->pwd, strlen(account->pwd), NULL), 16);
	...
	case -1:		// anonymous grant access
		...
		if (client[cs_idx].ncd_server) {
			cs_log("%s %s:%d-client %s%s (%s, %s)",
			       ...
			       client[cs_idx].ip ? cs_inet_ntoa(client[cs_idx].ip) : "",
			       client[cs_idx].ip ? t_grant : t_grant + 1, username(cs_idx), t_msg[rc]);
		} else {
...
int send_dcw(ECM_REQUEST * er)
{
	...
	cs_log("%s (%04X&%06X/%04X/%02X:%04X): %s (%d ms)%s",
	       ...
	       er->rcEx ? erEx : stxt[er->rc], 1000 * (tpe.time - er->tps.time) + tpe.millitm - er->tps.millitm, sby);
	er->caid = er->ocaid;
	switch (er->rc) {
	case 2:
	case 1:
...
//                  cs_log("chk_bcaid failed");
			...
			// moved behind the check routines, because newcamd-ECM will fail if ecm is converted before
		{
			...
			for (n = 0; (n < CS_MAXTUNTAB); n++)
				if ((er->caid == ttab->bt_caidfrom[n])
				    && ((er->srvid == ttab->bt_srvid[n]) || (ttab->bt_srvid[n]) == mask_all)) {
					int l;
					char hack_n3[13] =
					    { 0x70, 0x51, 0xc7, 0x00, 0x00, 0x00, 0x01, 0x10, 0x10, 0x00, 0x87, 0x12,
						0x07
					};
					char hack_n2[13] =
					    { 0x70, 0x51, 0xc9, 0x00, 0x00, 0x00, 0x01, 0x10, 0x10, 0x00, 0x48, 0x12,
						0x07
					};
...
//              er->reader[3], er->reader[4], er->reader[5]);
				if (act) {
					...
					if (!inc_stage) {
						...
						tpc.millitm +=
						    1000 * (tpn.time - er->tps.time) + tpn.millitm - er->tps.millitm;
						tpc.time += tpc.millitm / 1000;
						tpc.millitm = tpc.millitm % 1000;
					} else {
						er->locals_done = 0;
				...
				for (j = 0; j < ph[i].ptab->nports; j++) {
					if (ph[i].ptab->ports[j].fd && FD_ISSET(ph[i].ptab->ports[j].fd, &fds)) {
						if (ph[i].type == MOD_CONN_UDP) {
							...
							if ((n =
							     recvfrom(ph[i].ptab->ports[j].fd, buf + 3, sizeof(buf) - 3,
								      0, (struct sockaddr *) &cad,
								      (socklen_t *) & scad)) > 0) {
								int idx;
								idx =
								    idx_from_ip(cs_inet_order(cad.sin_addr.s_addr),
										ntohs(cad.sin_port));
								if (!idx) {
									if (pipe(fdp)) {
										cs_log("Cannot create pipe (errno=%d)",
										       errno);
										cs_exit(1);
									}
									switch (cs_fork
										(cs_inet_order(cad.sin_addr.s_addr),
										 ntohs(cad.sin_port))) {
									case -1:
										close(fdp[0]);
										close(fdp[1]);
...
//                    close(fdp[1]);    // now used to simulate event
										...
										client[cs_idx].udp_fd =
										    ph[i].ptab->ports[j].fd;
							...
							if ((pfd =
							     accept(ph[i].ptab->ports[j].fd, (struct sockaddr *) &cad,
								    (socklen_t *) & scad)) > 0) {
								switch (cs_fork
									(cs_inet_order(cad.sin_addr.s_addr),
									 ntohs(cad.sin_port))) {
