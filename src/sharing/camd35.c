...
static int camd35_send(uchar * buf)
{
	...
	if (is_udp)
		return (sendto(client[cs_idx].udp_fd, rbuf, l + 4, 0,
			       (struct sockaddr *) &client[cs_idx].udp_sa, sizeof(client[cs_idx].udp_sa)));
	else
...
static void casc_set_account()
{
	...
	memcpy(client[cs_idx].ucrc,
	       i2b(4, crc32(0L, MD5((unsigned char *) reader[ridx].r_usr, strlen(reader[ridx].r_usr), NULL), 16)), 4);
	aes_set_key((char *) MD5(upwd, strlen((char *) upwd), NULL));
	client[cs_idx].crypted = 1;
}
