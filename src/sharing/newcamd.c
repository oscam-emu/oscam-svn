...
static int network_message_send(int handle, uint16 * netMsgId, uint8 * buffer,
				int len, uint8 * deskey, comm_type_t commType, ushort sid)
{
...
static void network_cmd_no_data_send(int handle, uint16 * netMsgId,
				     net_msg_type_t cmd, uint8 * deskey, comm_type_t commType)
{
...
int network_tcp_connection_open(uint8 * hostname, uint16 port)
{
	...
	if (connect_nonb(client[cs_idx].udp_fd,
			 (struct sockaddr *) &client[cs_idx].udp_sa, sizeof(client[cs_idx].udp_sa), 5) < 0) {
...
static int connect_newcamd_server()
{
	...
	cs_log("CAID: %04X - UA: %02X%02X%02X%02X%02X%02X%02X%02X - Provider # %i", reader[ridx].caid[0],
	       reader[ridx].hexserial[0], reader[ridx].hexserial[1], reader[ridx].hexserial[2],
	       reader[ridx].hexserial[3], reader[ridx].hexserial[4], reader[ridx].hexserial[5],
	       reader[ridx].hexserial[6], reader[ridx].hexserial[7], buf[14 + 2]);
	...
	for (i = 0; i < reader[ridx].nprov; i++) {
		...
		cs_log("Provider ID: %02X%02X%02X - SA: %02X%02X%02X%02X", reader[ridx].prid[i][1],
		       reader[ridx].prid[i][2], reader[ridx].prid[i][3], reader[ridx].sa[i][0], reader[ridx].sa[i][1],
		       reader[ridx].sa[i][2], reader[ridx].sa[i][3]);
	}
...
static int newcamd_recv(uchar * buf, int l)
{
	...
	if (is_server)
		rs = network_message_receive(client[cs_idx].udp_fd,
					     &client[cs_idx].ncd_msgid, buf, client[cs_idx].ncd_skey, COMMTYPE_SERVER);
	else {
		if (!client[cs_idx].udp_fd)
			return (-1);
		rs = network_message_receive(client[cs_idx].udp_fd,
					     &reader[ridx].ncd_msgid, buf, reader[ridx].ncd_skey, COMMTYPE_CLIENT);
	}
	if (rs < 5)
		rc = (-1);
	else
					...
					if (!reader[r].ftab.nfilts) {
						...
						for (j = 0; !add && j < reader[r].ftab.nfilts; j++) {
							...
							if (!rcaid || rcaid == filt.caid) {
								for (k = 0; !add && k < reader[r].ftab.filts[j].nprids;
								     k++)
									if (reader[r].ftab.filts[j].prids[k] ==
									    psfilt->prids[i])
										add = 1;
							}
						}
				...
				if (client[cs_idx].ftab.filts[j].nprids) {
					for (k = 0; k < client[cs_idx].ftab.filts[j].nprids; k++)
						if (client[cs_idx].ftab.filts[j].prids[k] == psfilt->prids[i])
							filt.prids[filt.nprids++] =
							    client[cs_idx].ftab.filts[j].prids[k];
				} else {
					filt.prids[filt.nprids++] = psfilt->prids[i];
...
static int newcamd_auth_client(in_addr_t ip)
{
	...
	for (ok = 0, account = cfg->account; (usr) && (account) && (!ok); account = account->next) {
		...
		if (strcmp((char *) usr, account->usr) == 0) {
			...
			if (strcmp((char *) pwd, (const char *) passwdcrypt) == 0) {
				...
				cs_log("user %s authenticated successfully (using client %02X%02X)", usr, mbuf[0],
				       mbuf[1]);
				ok = 1;
				break;
			} else {
				cs_log("user %s is providing a wrong password (using client %02X%02X)", usr, mbuf[0],
				       mbuf[1]);
			}
		}
	}
	...
	if (!ok && !account) {
		cs_log("user %s is trying to connect but doesnt exist ! (using client %02X%02X)", usr, mbuf[0],
		       mbuf[1]);
		usr = 0;
	}
	...
	if (ok) {
		...
		if (au != -1) {
			if (cfg->ncd_ptab.ports[client[cs_idx].port_idx].ftab.filts[0].caid != reader[au].caid[0]
			    && cfg->ncd_ptab.ports[client[cs_idx].port_idx].ftab.filts[0].caid !=
			    reader[au].ftab.filts[0].caid) {
	...
	if (ok) {
		...
		if (i > 0) {
			...
			if (network_message_send
			    (client[cs_idx].udp_fd, &client[cs_idx].ncd_msgid, mbuf, len, key, COMMTYPE_SERVER,
			     0) < 0) {
				if (req) {
					free(req);
					req = 0;
...
static void newcamd_process_ecm(uchar * buf, int l)
{
	...
	cs_debug("ncd_process_ecm: er->cpti=%d, cl_msgid=%d, %02X", er->cpti, client[cs_idx].ncd_msgid, buf[2]);
	er->l = buf[4] + 3;
...
int newcamd_client_init()
{
	...
	cs_log("proxy %s:%d newcamd52%d (fd=%d%s)",
	       reader[ridx].device, reader[ridx].r_port, (ncd_proto == NCD_525) ? 5 : 4, client[cs_idx].udp_fd, ptxt);
