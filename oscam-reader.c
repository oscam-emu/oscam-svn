...
int hostResolve()
{
  ...
  in_addr_t last_ip = client[client[cs_idx].idx].ip;
  if (getaddrinfo(reader[client[cs_idx].ridx].device, NULL, &hints, &res) == 0) {
     client[cs_idx].udp_sa.sin_addr.s_addr = 
          ((struct sockaddr_in *)(res->ai_addr))->sin_addr.s_addr;
     client[cs_idx].ip = cs_inet_order(client[cs_idx].udp_sa.sin_addr.s_addr);
     
     if (client[client[cs_idx].idx].ip != last_ip) {
	      uchar *ip = (uchar*) &client[cs_idx].ip;
	      cs_debug("%s: resolved ip=%d.%d.%d.%d", reader[client[cs_idx].ridx].device, ip[3], ip[2], ip[1], ip[0]);
     }
  }
  else {
     client[cs_idx].udp_sa.sin_addr.s_addr = 0;
     cs_log("can't resolve %s", reader[client[cs_idx].ridx].device);
  }
