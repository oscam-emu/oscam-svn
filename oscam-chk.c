...
int chk_ctab(ushort caid, CAIDTAB *ctab) {
  ...
  {
    if (!ctab->caid[i]) {
      cs_debug_mask(D_TRACE, "CAID FILTERED1: %04x, reader %s", caid, reader[client[cs_idx].ridx].label);
      return 0;
    }
    if ((caid & ctab->mask[i]) == ctab->caid[i])
      return 1;
  }
  cs_debug_mask(D_TRACE, "CAID FILTERED2: %04x, reader %s", caid, reader[client[cs_idx].ridx].label);
  return 0;
}
